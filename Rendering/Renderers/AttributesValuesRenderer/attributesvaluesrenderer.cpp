#include "attributesvaluesrenderer.h"
...
AttrubutesValuesRenderer::AttrubutesValuesRenderer():Renderer("Properties Renderer")
{
	config = (AttrubutesValuesRendererConfig*)0;
	rendererWeight = RENDERER_WEIGHT_BASE+2.0f;
}
AttrubutesValuesRenderer::~AttrubutesValuesRenderer()
{
	if(config)
		delete config;
}
void AttrubutesValuesRenderer::glewIsReadyRenderer(){
	config = new AttrubutesValuesRendererConfig();
	ShaderLoadingData vertexShaderData(GL_VERTEX_SHADER);
	vertexShaderData.addFile("Rendering/Renderers/AttrubutesValuesRenderer/pvr.vert");
	ShaderLoadingData geometryShaderData(GL_GEOMETRY_SHADER);
	geometryShaderData.addFile("Rendering/Renderers/AttrubutesValuesRenderer/pvr.geom");
	ShaderLoadingData fragmentShaderData(GL_FRAGMENT_SHADER);
	fragmentShaderData.addFile("Rendering/Renderers/AttrubutesValuesRenderer/pvr.frag");
...
BaseRendererConfig* AttrubutesValuesRenderer::getRendererConfigWidget(){
	return config;
}

bool AttrubutesValuesRenderer::hasRendererConfigWidget(){
	return true;
}
void AttrubutesValuesRenderer::applyConfigChanges(RModel* rmodel){
	config->readConfig();
...
void AttrubutesValuesRenderer::createVertexPropertyData(RModel* rmodel){
	...
	// Here need to be code to set values to show on renderer (Attribute)
	 Function called for Property Renderer: config->evaluationStrategy->requestEvaluationResultsAsRModel(model, propertyData);
	/*
	if (nodes is selected)
		model->getAttributesNodes()
		propertyData need to be for vertex ?
	else // faces are selected
		propertyData.assign(model_>getAttributesFaces().size())

	*/
	

}
bool AttrubutesValuesRenderer::rmodelChanged(RModel* rmodel){
	config->readConfig();
	if(!rmodel || !rmodel->getOriginalModel()){
		//clear
		if(vertexPropertyValueBuffer!=RModel::NULL_BUFFER)
...
void AttrubutesValuesRenderer::draw(RModel* rmodel,CameraCamaron* camera){
...
REGISTER_RENDERER(AttrubutesValuesRenderer);
