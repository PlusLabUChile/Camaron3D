...
void IsosurfaceRenderer::draw(RModel* rmodel, CameraCamaron* camera){
	...
	glm::mat3 normalMatrix = glm::mat3(camera->getMV());

	ShaderUtils::setUniform(renderProgram, "MVP",camera->getMVP());
	...
	for(unsigned int i = 0; i< isosurfacesBuffers.size();i++) {
...
bool IsosurfaceRenderer::rmodelChanged(RModel* rmodel){
	...
	if(rmodel->bounds.size()<6
		...
		|| rmodel->getOriginalModel()->getPolyhedronsCount() == 0)
		return false;
