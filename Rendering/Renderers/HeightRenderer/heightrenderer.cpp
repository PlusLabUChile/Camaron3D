...
void HeightRenderer::draw(RModel* rmodel, CameraCamaron* camera){
	if(!rmodel)
		return;
	...
	ShaderUtils::setUniform(program,"u_TransformMatrix",camera->getMVP());
	...
	glVertexAttribPointer( 0, 3, GL_FLOAT, GL_FALSE, 0,
						   (GLubyte *)NULL );
	if(rmodel->getOriginalModel()->getPolygonsCount() == 0)//vis::CONSTANTS::VERTEX_CLOUD)
