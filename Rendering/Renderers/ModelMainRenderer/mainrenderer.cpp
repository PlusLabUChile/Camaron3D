...
bool MainRenderer::resizeFBO(RModel* rmodel, CameraCamaron* camera){
	if(!fboHandle || !rmodel)
		return false;
	glm::vec2 newSize = camera->getViewPortSize();
...
void MainRenderer::draw(RModel* rmodel, CameraCamaron* camera){
	...
	if(config->useDepthPeeling){
		drawWithDepthPeeling(rmodel, camera);
		return;
	}
...
void MainRenderer::drawWithDepthPeeling(RModel* rmodel, CameraCamaron* camera){
	if(!resizeFBO(rmodel, camera) || !depthPeelingProgram)
		return;
	fboHandle->bind();
	...
	for(int i = 0;i<config->depthPeelingLayers;i++){
		...
		ShaderUtils::setUniform(depthPeelingProgram, "MVP",camera->getMVP());
		ShaderUtils::setUniform(depthPeelingProgram, "ViewportSize",fboSize);
		ShaderUtils::setUniform(depthPeelingProgram, "ViewportMatrix",camera->getViewPortMatrix());
