...
void SelectionTableView::refreshSelectedElementsTable(){
	...
	while ( start<end )
	{
		...
		QStandardItem *rowPos2 = new QStandardItem(QString::fromStdString(ElementUtils::getPropertiesDisplay(It->second, selection)));
		rowPos2->setData(QString::fromStdString(ElementUtils::getPropertiesDisplay(It->second, selection)),0);
