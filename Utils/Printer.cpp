...
void Printer::print( Model* model, vis::Vertex* v ) {
...
void Printer::print( Model* model, vis::Polygon* p ) {
	std::cout << "Polygon " << p->getId() << ":\n";
	// std::vector<vis::Vertex*> &vecPol = p->getVertices();
	std::vector<unsigned int>& vecPol = model->getElementsRelations()->getVertexPolygonsById(p->getId());
	for( unsigned int vertex : vecPol ) {
		std::cout << "\t";
		Printer::print( model, &model->getVertices()[vertex] );
		std::cout << std::endl;
	}
...
void Printer::print( Model* model, vis::Element* p ) {
	std::cout << "Element ID: " << p->getId();
	if(p->isSelected())
		std::cout << " Selected";
	std::cout << std::endl;
}
void Printer::print( Model* model, Selection* sel ) {
