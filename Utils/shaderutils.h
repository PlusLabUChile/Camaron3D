...
class ShaderUtils
{
	public:
        ...
        template <class myType>
        static bool recoverDataBuffer(GLuint bufferHandle, std::vector<myType>&);
...
template <class myType>
bool ShaderUtils::recoverDataBuffer(GLuint bufferHandle, std::vector<myType>& vector){
    if(!bufferHandle)
		return false;
    glBindBuffer(GL_ARRAY_BUFFER, bufferHandle);
    glGetBufferSubData(GL_ARRAY_BUFFER, 0, sizeof(myType)*vector.size(), vector.data());
    glBindBuffer(GL_ARRAY_BUFFER, 0);
    return true;
}
#endif // SHADERUTILS_H
