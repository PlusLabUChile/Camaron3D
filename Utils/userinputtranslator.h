...
#define ADD_USER_INPUT_DIC_UNIT_2INPUTS(category,description,code,key, mkey, shift, control)	  \
{															 \
	UserInputTranslationUnit userinputunit(code,key,mkey,shift,control,\
	std::string(category),std::string(description));\
	if(keysDictionary.count(category))\
		keysDictionary[category].push_back(userinputunit);\
	else{\
...
class UserInputTranslator//singleton
{
	public:
		...
		///@unused bool getConfigFromFile(std::string);
		///@noop
